...
def cnn(input_shape: Tuple[int, ...]) -> nn.Module:
    ...
    class Classifier(nn.Module):
        def __init__(self):
            ...
            dim_w = (input_w - 2 - 2) / 2   # [(W−K+2P)/S]+1 for each conv, /2 for maxpool
            dim_h = (input_h - 2 - 2) / 2   # [(W−K+2P)/S]+1 for each conv, /2 for maxpool
